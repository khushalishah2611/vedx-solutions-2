...
const TermsConditionPage = () => {
  ...
  const { banner } = useBannerByType('terms-and-condition');
  const [terms, setTerms] = useState(null);
  const [error, setError] = useState('');

  const activeTab = useMemo(() => tabs[1].path, []);
  ...
  useEffect(() => {
    ...
    const loadTerms = async () => {
      setError('');
      try {
        const response = await fetchWithLoading(apiUrl('/api/terms-conditions'));
        const payload = await response.json();
        if (!response.ok) throw new Error(payload?.message || 'Unable to load terms right now.');
        if (!isMounted) return;
        setTerms(payload?.terms || null);
      } catch (err) {
        if (!isMounted) return;
        setError(err?.message || 'Unable to load terms right now.');
        setTerms(null);
      }
    };
  ...
  const heroTitle = banner?.title || 'Terms and Condition';
        ...
        <Container maxWidth={false} sx={{ px: { xs: 3, md: 20 } }}>
          <Stack spacing={2} alignItems="center">
            ...
            <Typography variant="body1" sx={{ color: alpha('#fff', 0.85), maxWidth: 720 }}>
              Understand the terms that guide our collaboration and service commitments.
            </Typography>
          </Stack>
        </Container>
      </Box>
          ...
          >
            {terms?.description ? (
              ...
              <Typography color="text.secondary" align="center">
                Terms content will appear here once it is published.
              </Typography>
            )}
          </Box>
        )}
