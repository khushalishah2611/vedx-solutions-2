// TermsConditionPage.jsx ✅ (SINGLE FILE) - NO LOADER, NO ALERT
...
const BANNER_TYPE = "terms-and-condition";
...
const TermsConditionPage = () => {
  ...
  const activeTab = useMemo(() => {
    const current = location?.pathname || TABS[1].path;
    return TABS.some((t) => t.path === current) ? current : TABS[1].path;
  }, [location?.pathname]);
  ...
  useEffect(() => {
    ...
    const loadTerms = async () => {
      try {
        ...
        const doc = payload?.terms || payload?.data || payload?.result || null;
  ...
  const heroDescription =
    banner?.description ||
    banner?.subtitle ||
    "Please read these terms carefully to understand your rights, responsibilities, and the conditions for using our services.";
